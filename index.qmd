...
#### C. Téléchargement {.unnumbered}

Les données utilisées dans cet article, annexes incluses, sont mises à disposition dans une archive téléchargeable à [**ce lien**](https://rzine.fr/docs/20251001_classification_regionalisation/data.zip).
...
# Passage en logarithme du % national 
...
map_circ <- map |> 
              ...
              select(circ) |>
              left_join (meta) |>     
              filter(nchar(dept)<3, !dept %in% c("2A", "2B")) |> 
...
On vérifie tout d'abord que la procédure d'agrégation a bien donné les mêmes totaux au niveau des circonscriptions, départements et régions. Il apparaît que pour chaque niveau le nombre total d'inscrits est bien le même et il ne semble pas utile de vérifier les autres colonnes.
