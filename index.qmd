...
## Classification  et régionalisation {.unnumbered}
...
mf_map(map_border_dep,
       ...
       leg_pos = "left",
       add = TRUE)
mf_layout("Relation entre régionalisation et discontinuités", frame = TRUE)
...
**La région Nord-Est (NE)** se caractérisent par une surreprésentation générale des votes pour les listes de droite (Bellamy) ou d'extrême-droite (Bardella, Maréchal) dans trois de ses composantes sous-régionales, auxquelles il faut ajouter une enclave.
...
cartogram_R <- cartogramR(mapdon_circ,
                          count = "vot",
                          method = "dcn")$cartogram |>
                st_as_sf()

### CG to HP : j'ai supprimé les options qui provoquaient une erreur ###
...
# Découpage en 22 classes
mapdon_circ$reg22 <- as.factor(cutree(regio, 22))
cartogram_R$reg22 <- as.factor(cutree(regio, 22))
...
Il n'est pas besoin ici de faire appel à un processus de diffusion pour expliquer des différences de comportements politiques qui peuvent s'expliquer par plus de trois décennies d'appartenance à des régions ayant connu des évolutions politiques différrentes.



## Régionaliser l'espace et le temps {.unnumbered}
